...
app.post("/register", validationRule.form, (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    //console.log(errMsg);
    res.status(600).render("registration", { errors: errMsg });
  } else {
    serverDataModule
      .register(req.body)
      .then(() => {
        var msg = "User Created!";
        res.status(200).render("registration", { errors: errMsg, msg });
      })
      .catch((err) => {
        var msg = err;
        res.status(500).render("registration", { errors: errMsg, msg });
      });
  }
});
//Login Route
